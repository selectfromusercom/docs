...
| 방식 | 설명 | 적합한 경우 |
|------|------|-------------|
| **앱(App) 공유** | 셀렉트 앱에 로그인 후, 여러 어드민을 전환하며 사용 | 한 명이 여러 어드민을 동시에 사용하는 경우 |
| **셀렉트 도메인 공유** | `domain.selectfromuser.com` 형태의 주소를 직접 공유 | 별도 도메인 설정 없이 빠르게 하나만 공유하고 싶은 경우 |
| **자사 커스텀 도메인** | `admin.mydomain.com` 형태의 자사 도메인을 연결 | 브랜드 일관성이 중요하거나, 고객사 도메인으로 노출하고 싶은 경우 |
